...
<form class="mt-6" method="POST">
	...
	{#if form?.success === false}<p class="text-red-500 text-center mt-4">{form?.message}</p>{/if}
	{#if form?.success === true}<p class="text-green-500 text-center mt-4">Login e-mail sent. Please check your inbox.</p>{/if}
</form>
